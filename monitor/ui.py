...
def render_snapshot(snapshot: SystemSnapshot, trends: dict[str, str] | None = None) -> str:
    ...
    lines = [
        ...
        f"Net Up:   {_format_rate(snapshot.net_sent_rate_smoothed)}",
        f"Net Down: {_format_rate(snapshot.net_recv_rate_smoothed)}",
        f"Net Up Raw:   {_format_rate(snapshot.net_sent_rate)}",
        f"Net Down Raw: {_format_rate(snapshot.net_recv_rate)}",
